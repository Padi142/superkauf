...
class PostsRepository {
  ...
  Future<GetPostsResult> getPosts(GetPostsBody body) async {
    return postApi
        .getFeed(body: body.pagination.toJson(), country: body.country)
        .then((posts) {
  ...
  Future<GetPersonalFeedResult> getPersonalFeed(
      GetPostsBody body, int id) async {
    return postApi
        .getPersonalFeed(
            body: body.pagination.toJson(), id: id, country: body.country)
        .then((posts) {
      return GetPersonalFeedResult.success(posts);
    }).onError((error, stackTrace) {
      if (error is DioException) {
        return GetPersonalFeedResult.failure(
  ...
  Future<GetPostsResult> getPostsByUser(
      GetPostsPaginationModel body, int userID) async {
    return postApi
        .getPostsByUser(
            id: userID.toString(), per_page: body.perPage, offset: body.offset)
        .then((posts) {
      return GetPostsResult.success(GetPostsResponseModel(
          posts: posts,
          pagination: const PaginationModel(count: 1, perPage: 1)));
    }).onError((error, stackTrace) {
      if (error is DioException) {
        return GetPostsResult.failure(error.message ?? 'error getting posts');
      }
  ...
  Future<GetFullContextPostDetailResult> getPostDetail(
      String postId, int userId) async {
    return postApi.getPostById(id: postId, userId: userId).then((post) {
      return GetFullContextPostDetailResult.success(post);
    }).onError((error, stackTrace) {
      if (error is DioException) {
        return GetFullContextPostDetailResult.failure(
            error.message ?? 'error getting posts');
      }
  ...
  Future<GetPostDetailResult> updatePostContent(UpdatePostBody body) async {
    return postApi
        .updatePostContent(body: body.toJson(), field: 'description')
        .then((post) {
      return GetPostDetailResult.success(post);
    }).onError((error, stackTrace) {
      if (error is DioException) {
  ...
  Future<GetPostDetailResult> updatePostValidUntil(
      UpdatePostValidUntilBody body) async {
    return postApi
        .updatePostContent(body: body.toJson(), field: 'valid_until')
        .then((post) {
      return GetPostDetailResult.success(post);
    }).onError((error, stackTrace) {
      if (error is DioException) {
  ...
  Future<ReactionsResult> addReaction(AddReactionModel body) async {
    return postApi
        .addReaction(body: body.toJson(), id: body.post.toString())
        .then((reaction) {
      return ReactionsResult.success(reaction);
    }).onError((error, stackTrace) {
      if (error is DioException) {
        return ReactionsResult.failure(
            error.message ?? 'error adding reaction');
      }
  ...
  Future<ReactionsResult> removeReaction(RemoveReactionModel body) async {
    return postApi
        .removeReaction(body: body.toJson(), id: body.post.toString())
        .then((reaction) {
      return ReactionsResult.success(reaction);
    }).onError((error, stackTrace) {
      if (error is DioException) {
        return ReactionsResult.failure(
            error.message ?? 'error removing reaction');
      }
      return ReactionsResult.failure(error.toString());
    });
  }

  Future<GetPersonalFeedResult> getTopPosts(
      GetPostsBody body, int userID, String timeRange) async {
    return postApi
        .getTopPosts(
